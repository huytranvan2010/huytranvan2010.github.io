...
$G^{*}$ mà tối ưu nhất sẽ cho $p_g(x)$ trùng với $p_r(x)$ thì khoảng cách Jensen-Johnson $D_{JS}(p_r(x) \Vert p_g(x)) = 0$ và khi đó $L(G, D^*) = -2\log2$ như chúng ta sẽ nhận được ở bên dưới. 
...
$K$ - hằng số Lipschitz cho hàm $f$. Hàm số khả vi, liên tục trên toàn miên xác định thì liên tục Lipschitz, bởi vì đạo hàm được ước lượng thông qua $\frac{|f(x_1) - f(x_2)|}{|x_1 - x_2|}$ bị chặn. Tuy nhiên hàm số liên tục Lipschitz chưa chắc đã khả vi trên toàn miền, ví dụ $f(x) = |x|$.
